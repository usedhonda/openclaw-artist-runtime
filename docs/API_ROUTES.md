...
| `POST` | `/api/run-cycle` | `plugin` | Manually trigger one autopilot cycle. | optional `config`; set `operatorRequestedSpawn: true` for an explicit one-off song proposal; set optional `forceObservationRefresh: true` to bypass cached observation reuse for this cycle | autopilot state plus `tickerOutcome` and `tickerLastTickAt` | An explicit producer request bypasses the autonomous recent-completion cooldown and, when no song lane is active, the proposal interval. Forced observation refresh is limited to this cycle. All Suno and publishing boundaries remain unchanged. |
...
| `POST` | `/api/songs/:songId/abandon` | `plugin` | Operator lane-clear: mark a non-terminal song `failed` (`abandoned_by_operator`) and free the autopilot `currentSongId` lane. | optional `config` | abandon result with body `statusCode` | Refuses with `409` when the song is terminal or has an accepted Suno run; appends an audit entry and emits `song_abandoned`. |
| `POST` | `/api/songs/:songId/attach-takes` | `plugin` | Operator recovery: attach feed-verified take URLs to a song whose manual Suno create was not recorded (e.g. an unavailable feed rejected a real create). | `urls: string[]`, optional `reason`, optional `config` | attach result with body `statusCode` | Rejects non-UUID URLs (`400`), an unknown song (`404`), a terminal song or a take-attribution collision (`409`), a title mismatch against the authenticated feed (`422`), or an unreachable feed (`503`); accepts `failed` songs (the parked-by-3-strikes state) since that is the lane this route exists to recover; on success appends an accepted `SunoRunRecord`, a `take_attribution_operator_attach` audit entry, and emits `suno_take_attached_by_operator`. |
