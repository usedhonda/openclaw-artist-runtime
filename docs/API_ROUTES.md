...
| `POST` | `/api/run-cycle` | `plugin` | Manually trigger one autopilot cycle. | optional `config`; set `operatorRequestedSpawn: true` for an explicit one-off song proposal | autopilot state plus `tickerOutcome` and `tickerLastTickAt` | An explicit producer request bypasses the autonomous recent-completion cooldown and, when no song lane is active, the proposal interval. All Suno and publishing boundaries remain unchanged. |
