...
| `POST` | `/api/run-cycle` | `plugin` | Manually trigger one autopilot cycle. | optional `config`; set `operatorRequestedSpawn: true` for an explicit one-off song proposal | autopilot state plus `tickerOutcome` and `tickerLastTickAt` | An explicit producer request bypasses only the autonomous recent-completion proposal cooldown; all Suno and publishing boundaries remain unchanged. |
