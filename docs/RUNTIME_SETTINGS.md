...
| `aiReview.provider` | Schema config, env forced | Supported values are `mock`, `openclaw`, and `openai-codex`. `OPENCLAW_AI_REVIEW_PROVIDER` can force the effective provider and makes the field read-only in Settings. In the hosted plugin, real creative calls use OpenClaw's public runtime, inheriting its configured model, auth, and reasoning policy without reading auth files; tools are disabled and no delivery is requested. Standalone runs without a host runtime retain the legacy `openai-codex` auth-file path. |
