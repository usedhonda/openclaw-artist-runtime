...
| `aiReview.provider` | Schema config, env forced | Supported values are `mock`, `openclaw`, and `openai-codex`. `OPENCLAW_AI_REVIEW_PROVIDER` can force the effective provider and makes the field read-only in Settings. For `openai-codex`, the model is always OpenClaw's `agents.defaults.model.primary`, and reasoning defaults to OpenClaw's `agents.defaults.thinkingDefault` when it is `low`, `medium`, `high`, or `xhigh`; lyric calls inherit that common default, while song-proposal calls explicitly use `xhigh`. Artist Runtime has no separate model setting and refuses the call when that primary is absent. |
