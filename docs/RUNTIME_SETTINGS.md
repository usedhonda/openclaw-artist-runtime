...
| `music.suno.driver`, `music.suno.connectionMode` | Schema config, env forced | `OPENCLAW_SUNO_LIVE` and `OPENCLAW_SUNO_DRIVER` can force the live connection and driver values. |
| `music.suno.submitMode` | Schema config, env forced only when explicit | Editable in Settings even when `OPENCLAW_SUNO_LIVE=on`: `manual` (off) waits for producer edits and a manual Create click; `live` (on) presses Create automatically. An explicit `OPENCLAW_SUNO_SUBMIT_MODE` still forces the field read-only. |
...
| `music.suno.audioImport` | Schema config | `download` (default) fetches accepted take audio through the connector. `skip` records the take URLs only and never downloads audio: the pending-import sweep and the post-adoption download job are bypassed, the song moves to `takes_imported` by reference, and the public Suno URL stays its only asset. Use when the Suno download quota is limited. |
...
| `aiReview.provider` | Schema config, env forced | Supported values are `mock`, `openclaw`, and `openai-codex`. `OPENCLAW_AI_REVIEW_PROVIDER` can force the effective provider and makes the field read-only in Settings. In the hosted plugin, real creative calls use the public `runtime.llm.complete` facade first, inheriting the configured model/auth and passing the host `agents.defaults.thinkingDefault`; no tools or delivery are requested. Standalone runs without a host runtime retain the legacy `openai-codex` auth-file path. |
