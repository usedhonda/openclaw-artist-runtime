...
Status: monitoring after corrective fix (archived stale, superseded by Linux host parity work)
...
---
Archived 2026-09-06: this handoff went stale (the Mac launchd-based gateway
lifecycle it describes is being retired for Linux host parity). The final
corrective-observation completion condition was never explicitly confirmed
closed before work moved on; if that 24-hour stability question resurfaces on
the Linux host, re-open it as a new dated handoff rather than reviving this
one.
