...
import { dailySharingModes, instagramAuthorityModes, officialReleaseModes, tiktokAuthorityModes, xAuthorityModes } from "../types.js";
...
export async function producerConsoleHtml(projectRoot = PLUGIN_ROOT): Promise<string> {
  ...
  const dailySharingOptions = authorityOptions(dailySharingModes);
  const officialReleaseOptions = authorityOptions(officialReleaseModes);
  ...
  return [
    ...
    "</section>",
    "<section class=\"grid\" style=\"margin-top:16px\">",
    `<article class="panel"><div class="muted">Config Editor</div><form id="config-form" class="config-form"><label class="toggle"><input id="cfg-autopilot-enabled" type="checkbox" />Autopilot enabled</label><label class="toggle"><input id="cfg-dry-run" type="checkbox" />Dry-run safety</label><div id="cfg-dry-run-warning" class="warning" hidden>Dry-run is OFF. The runtime stays fail-closed, but this arm can permit live side effects if the connectors are ready.</div><div class="field-grid"><label><div class="muted">Dashboard URL</div><input id="cfg-dashboard-base-url" type="url" placeholder="https://your-tailnet-host:43134" /></label><label><div class="muted">Songs Per Week</div><input id="cfg-songs-per-week" type="number" min="0" max="100" /></label><label><div class="muted">Cycle Interval Minutes</div><input id="cfg-cycle-interval" type="number" min="15" max="1440" /></label></div><div class="field-grid"><label><div class="muted">Daily Suno credit limit</div><input id="cfg-daily-credit-limit" type="number" min="1" max="1000" /></label><label><div class="muted">Monthly Suno credit limit</div><input id="cfg-monthly-credit-limit" type="number" min="0" max="50000" /></label><label><div class="muted">Monthly Suno generation budget</div><input id="cfg-monthly-generation-budget" type="number" min="0" max="1000" /></label><label><div class="muted">Max generations per day</div><input id="cfg-max-generations-per-day" type="number" min="0" max="100" /></label><label><div class="muted">Minutes between creates</div><input id="cfg-min-minutes-between-creates" type="number" min="1" max="1440" /></label></div><div class="field-grid"><label><div class="toggle"><input id="cfg-song-spawn-enabled" type="checkbox" />Autonomous song ideas</div><div class="muted">Idea interval hours</div><input id="cfg-song-spawn-hours" type="number" min="12" max="168" /></label><label><div class="toggle"><input id="cfg-artist-pulse-enabled" type="checkbox" />Artist pulse</div><div class="muted">Pulse interval hours</div><input id="cfg-artist-pulse-hours" type="number" min="6" max="168" /></label><label class="toggle"><input id="cfg-commission-enabled" type="checkbox" />Producer commissions</label></div><div class="field-grid"><label><div class="toggle"><input id="cfg-distribution-enabled" type="checkbox" />Social distribution enabled</div><div class="toggle"><input id="cfg-distribution-live-go-armed" type="checkbox" />Allow public publishing globally</div></label><label><div class="muted">Daily sharing</div><select id="cfg-daily-sharing">${dailySharingOptions}</select></label><label><div class="muted">Official release</div><select id="cfg-official-release">${officialReleaseOptions}</select></label></div><div class="field-grid"><label><div class="toggle"><input id="cfg-x-enabled" type="checkbox" />X enabled</div><div class="muted">X Authority</div><select id="cfg-x-authority">${xAuthorityOptions}</select></label><label><div class="toggle"><input id="cfg-instagram-enabled" type="checkbox" />Instagram enabled</div><div class="muted">Instagram Authority</div><select id="cfg-instagram-authority">${instagramAuthorityOptions}</select></label><label><div class="toggle"><input id="cfg-tiktok-enabled" type="checkbox" />TikTok enabled</div><div class="muted">TikTok Authority</div><select id="cfg-tiktok-authority">${tiktokAuthorityOptions}</select></label></div><div class="muted" id="cfg-meta"></div><div id="config-error" class="field-error"></div><div class="actions"><button class="primary" id="config-save" type="submit">Save Settings</button><button id="config-reset" type="button">Reset Draft</button><button id="config-refresh" type="button">Refresh</button></div></form></article>`,
    "</section>",
    ...
    `const dailySharingModes=${JSON.stringify(dailySharingModes)};`,
    `const officialReleaseModes=${JSON.stringify(officialReleaseModes)};`,
    ...
    "function syncConfigForm(config){if(configDirty) return; document.getElementById('cfg-autopilot-enabled').checked=Boolean(config.autopilot.enabled); document.getElementById('cfg-dry-run').checked=Boolean(config.autopilot.dryRun); document.getElementById('cfg-dashboard-base-url').value=String(config.dashboard?.baseUrl ?? ''); document.getElementById('cfg-songs-per-week').value=String(config.autopilot.songsPerWeek ?? 0); document.getElementById('cfg-cycle-interval').value=String(config.autopilot.cycleIntervalMinutes ?? 180); document.getElementById('cfg-daily-credit-limit').value=String(config.music.suno.dailyCreditLimit ?? 60); document.getElementById('cfg-monthly-credit-limit').value=String(config.music.suno.monthlyCreditLimit ?? 0); document.getElementById('cfg-monthly-generation-budget').value=String(config.music.suno.monthlyGenerationBudget ?? 50); document.getElementById('cfg-max-generations-per-day').value=String(config.music.suno.maxGenerationsPerDay ?? 4); document.getElementById('cfg-min-minutes-between-creates').value=String(config.music.suno.minMinutesBetweenCreates ?? 20); document.getElementById('cfg-song-spawn-enabled').checked=Boolean(config.songSpawn?.enabled); document.getElementById('cfg-song-spawn-hours').value=String(config.songSpawn?.minIntervalHours ?? 24); document.getElementById('cfg-artist-pulse-enabled').checked=Boolean(config.artistPulse?.enabled); document.getElementById('cfg-artist-pulse-hours').value=String(config.artistPulse?.minIntervalHours ?? 12); document.getElementById('cfg-commission-enabled').checked=Boolean(config.commission?.enabled); document.getElementById('cfg-distribution-enabled').checked=Boolean(config.distribution.enabled); document.getElementById('cfg-distribution-live-go-armed').checked=Boolean(config.distribution.liveGoArmed); document.getElementById('cfg-daily-sharing').value=String(config.distribution.dailySharing ?? 'auto'); document.getElementById('cfg-official-release').value=String(config.distribution.officialRelease ?? 'manual_approval'); document.getElementById('cfg-x-enabled').checked=Boolean(config.distribution.platforms.x.enabled); document.getElementById('cfg-x-authority').value=String(config.distribution.platforms.x.authority ?? 'draft_only'); document.getElementById('cfg-instagram-enabled').checked=Boolean(config.distribution.platforms.instagram.enabled); document.getElementById('cfg-instagram-authority').value=String(config.distribution.platforms.instagram.authority ?? 'draft_only'); document.getElementById('cfg-tiktok-enabled').checked=Boolean(config.distribution.platforms.tiktok.enabled); document.getElementById('cfg-tiktok-authority').value=String(config.distribution.platforms.tiktok.authority ?? 'draft_only'); text('cfg-meta',`artist ${config.artist.artistId} · workspace ${config.artist.workspaceRoot}`); document.getElementById('cfg-dry-run-warning').hidden=Boolean(config.autopilot.dryRun); text('config-error','');}",
    "function buildConfigPatch(){const dashboardBaseUrl=value('cfg-dashboard-base-url').trim(); const songsPerWeek=Number(value('cfg-songs-per-week')); const cycleIntervalMinutes=Number(value('cfg-cycle-interval')); const dailyCreditLimit=Number(value('cfg-daily-credit-limit')); const monthlyCreditLimit=Number(value('cfg-monthly-credit-limit')); const monthlyGenerationBudget=Number(value('cfg-monthly-generation-budget')); const maxGenerationsPerDay=Number(value('cfg-max-generations-per-day')); const minMinutesBetweenCreates=Number(value('cfg-min-minutes-between-creates')); const songSpawnMinIntervalHours=Number(value('cfg-song-spawn-hours')); const artistPulseMinIntervalHours=Number(value('cfg-artist-pulse-hours')); const dailySharing=value('cfg-daily-sharing'); const officialRelease=value('cfg-official-release'); const xAuthority=value('cfg-x-authority'); const instagramAuthority=value('cfg-instagram-authority'); const tiktokAuthority=value('cfg-tiktok-authority'); if(!Number.isInteger(songsPerWeek)||songsPerWeek<0||songsPerWeek>100) throw new Error('songsPerWeek must be between 0 and 100'); if(!Number.isInteger(cycleIntervalMinutes)||cycleIntervalMinutes<15||cycleIntervalMinutes>1440) throw new Error('cycleIntervalMinutes must be between 15 and 1440'); if(!Number.isInteger(dailyCreditLimit)||dailyCreditLimit<1||dailyCreditLimit>1000) throw new Error('dailyCreditLimit must be between 1 and 1000'); if(!Number.isInteger(monthlyCreditLimit)||monthlyCreditLimit<0||monthlyCreditLimit>50000) throw new Error('monthlyCreditLimit must be between 0 and 50000'); if(!Number.isInteger(monthlyGenerationBudget)||monthlyGenerationBudget<0||monthlyGenerationBudget>1000) throw new Error('monthlyGenerationBudget must be between 0 and 1000'); if(!Number.isInteger(maxGenerationsPerDay)||maxGenerationsPerDay<0||maxGenerationsPerDay>100) throw new Error('maxGenerationsPerDay must be between 0 and 100'); if(!Number.isInteger(minMinutesBetweenCreates)||minMinutesBetweenCreates<1||minMinutesBetweenCreates>1440) throw new Error('minMinutesBetweenCreates must be between 1 and 1440'); if(!Number.isInteger(songSpawnMinIntervalHours)||songSpawnMinIntervalHours<12||songSpawnMinIntervalHours>168) throw new Error('songSpawnMinIntervalHours must be between 12 and 168'); if(!Number.isInteger(artistPulseMinIntervalHours)||artistPulseMinIntervalHours<6||artistPulseMinIntervalHours>168) throw new Error('artistPulseMinIntervalHours must be between 6 and 168'); if(!dailySharingModes.includes(dailySharing)) throw new Error('dailySharing must be one of the supported modes'); if(!officialReleaseModes.includes(officialRelease)) throw new Error('officialRelease must be one of the supported modes'); if(!xAuthorityModes.includes(xAuthority)) throw new Error('xAuthority must be one of the supported X authority modes'); if(!instagramAuthorityModes.includes(instagramAuthority)) throw new Error('instagramAuthority must be one of the supported Instagram authority modes'); if(!tiktokAuthorityModes.includes(tiktokAuthority)) throw new Error('tiktokAuthority must be one of the supported TikTok authority modes'); return {dashboard:{baseUrl:dashboardBaseUrl},music:{suno:{dailyCreditLimit,monthlyCreditLimit,monthlyGenerationBudget,maxGenerationsPerDay,minMinutesBetweenCreates}},autopilot:{enabled:checked('cfg-autopilot-enabled'),dryRun:checked('cfg-dry-run'),songsPerWeek,cycleIntervalMinutes},artistPulse:{enabled:checked('cfg-artist-pulse-enabled'),minIntervalHours:artistPulseMinIntervalHours},commission:{enabled:checked('cfg-commission-enabled')},songSpawn:{enabled:checked('cfg-song-spawn-enabled'),minIntervalHours:songSpawnMinIntervalHours},distribution:{enabled:checked('cfg-distribution-enabled'),liveGoArmed:checked('cfg-distribution-live-go-armed'),dailySharing,officialRelease,platforms:{x:{enabled:checked('cfg-x-enabled'),authority:xAuthority},instagram:{enabled:checked('cfg-instagram-enabled'),authority:instagramAuthority},tiktok:{enabled:checked('cfg-tiktok-enabled'),authority:tiktokAuthority}}}};}",
