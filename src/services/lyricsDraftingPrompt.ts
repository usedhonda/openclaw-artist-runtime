...
// The section budget table is rendered from the song's own DurationPlan. It used to
// be a hardcoded mid-band table, so a fast song's prompt carried two contradictory
// verse lengths: this static "各16 bars" and the real plan's 22-bar verse.
function sectionBudgetTable(plan: DurationPlan): string {
  return plan.sectionPlan
    .map((section) => `| ${section.label} | ${section.lineTarget} | ${section.bars} bars |`)
    .join("\n");
}

function durationAlignedLyricsWriterSystemPrompt(plan: DurationPlan): string {
  return LYRICS_WRITER_SYSTEM_PROMPT
  .replace(
    "**デフォルトの曲尺: 4-5分相当。** 指定がない限り、以下の構成と行数予算で書く：",
...
export function buildLyricsDraftingPrompt(input: BuildLyricsPromptInput): string {
  ...
  return [
    ...
    durationAlignedLyricsWriterSystemPrompt(durationPlan),
    "",
    ...
    "Japanese rhyme blueprint: before drafting each rap verse, choose one or two 2-4 mora vowel-chain anchors. In every four-bar unit, place at least one multisyllabic or compound-rhyme pair, at least two internal-rhyme echoes, and carry one bar-end vowel chain across two or more lines. Change cadence after each four-bar unit and include at least one Japanese wordplay, double meaning, or semantic turn per verse. Make the rhyme audible without printing a rhyme analysis in the lyric output.",
    "Meaning first: write the intended Japanese image or claim, identify its vowel chain, then tune consonants and word order. Never bend syntax into unnatural Japanese merely to satisfy a rhyme; replace the rhyme family instead.",
    ...
    "Place-name rule: let the city register through concrete evidence from the observation and persona material bank: a landmark such as いちまるきゅう (never numeric 109 in lyric text), a redevelopment route, station gates, tax-free bags, clinic ads, rent, height, or a storefront detail. Do not use 渋谷 / Shibuya as a recurring hook, refrain, or default label. Use the explicit place name only when one line materially needs it, and at most once in the lyric body.",
    "Indirect-diss rule: reveal the target through a fresh transformed image before naming it. For a decaying district, invent one song-specific metaphor family such as 腐った谷, 光る空き箱, 顔を貼り替える壁, or 同じ声を吐く看板, then develop concrete scenes inside that family. These are examples, not reusable catchphrases: do not copy the same metaphor family into the next song. Do not explain the metaphor after landing it.",
    "Advertising rule: show what the ad does to eyes, walls, faces, routes, rent, or attention; do not repeatedly say 広告 or turn the lyric into a list of ad categories. Let the object or effect carry the accusation, and reserve a direct target name for a single reveal or punchline when it materially deepens the verse.",
    ...
    "Keep vocal pacing dense but controlled; if double-density is used, confine it to 2-4 bar bursts and never make it the full-song default.",
