...
export function buildLyricsDraftingPrompt(input: BuildLyricsPromptInput): string {
  ...
  return [
    ...
    "Opening rule: invent this song's opening from its particular observation, rather than selecting a stock intro pattern. An opening has only two valid renderable forms: [Instrumental Intro - concrete sound/action, no vocals] with zero lyric lines, or [Intro - concrete delivery/action] with exactly one complete intelligible lyric line. Never emit an empty Intro, a count-in, phonetic filler, scat, vocal chops, ad-libs, or a standalone production cue such as [processed vocals]. Keep fast/double-density delivery inside Verse sections, never before the first written lyric line.",
    ...(input.decision
