...
export const STYLE_SYNTHESIS_SYSTEM_PROMPT = STYLE_ANALYZER_SYSTEM_PROMPT
  .replace("Style (English only, 700 chars max)", "Style (English only, canonical short form)")
  .replace("**Target: 900-1000 characters. Absolute limit: 1000 characters.**", "**Canonical target: core <=120 characters, total <=400 characters. Absolute limit: 1000 characters.**")
  .replace("**USE the full space.** Be detailed and specific based on the URL investigation. Do NOT be brief. Expand each section with rich, specific descriptions drawn from the reference track analysis. Every instrument, mix characteristic, and arrangement detail you discovered should be reflected.", "**Do not pad.** Keep genre/mood/vocal/instrument anchors, but keep Style subordinate to DurationPlan/YAML section tags.")
  .replace("If over 1000, cut Arrangement Notes first, then Texture.", "If over 400, cut Arrangement Notes first, then Texture. If over 1000, regenerate.")
  .replace("Style is 100% English, ≤700 chars", "Style is 100% English, core <=120 chars, target <=400 chars, hard <=1000 chars");
...
export async function buildStyleSynthesisPrompt(input: BuildStyleInput): Promise<StyleSynthesisPrompt> {
  ...
  const user = [
    ...
    "Length budget: 4-7 front-loaded descriptors, core <=120 characters, total target <=400 characters, absolute upper bound 1000. Do not pad or expand to fill space.",
