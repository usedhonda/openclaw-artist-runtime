...
const STYLE_KNOWLEDGE_BUDGETS: Record<string, number> = {
  ...
  "suno_v6_reference.md": 5000,
  "v55_to_v6_migration.md": 3000,
...
export async function buildStyleSynthesisPrompt(input: BuildStyleInput): Promise<StyleSynthesisPrompt> {
  ...
  const user = [
    ...
    "Primary genre identity must be unmistakable. If secondary genres appear, assign each one a musical job instead of stacking labels.",
    "Specify playing behavior, vocal behavior, groove/pocket, section contrast, and mix hierarchy. A distinct BPM is required when tempo is available.",
    ...
    optionalLine("Emotional mode (感情)", input.emotionalModeSpec),
    optionalLine("Producer style notes", input.styleNotes),
    optionalLine("Intro move", input.introStyleMove),
  ...
  return {
    ...
    system: STYLE_SYNTHESIS_SYSTEM_PROMPT,
    user: user.join("\n")
  };
}
