...
export const STYLE_SYNTHESIS_SYSTEM_PROMPT = STYLE_ANALYZER_SYSTEM_PROMPT
  // Style is the primary quality signal — keep the analyzer's "use the full space"
  // guidance instead of the old "canonical short" (<=400, do not pad) override that
  // made every track thin. Target a rich 760-900 chars under the 1000 hard ceiling.
  .replace("Style (English only, 700 chars max)", "Style (English only, rich detailed form)")
  .replace("**Target: 900-1000 characters. Absolute limit: 1000 characters.**", "**Target: 760-900 characters. Absolute limit: 1000 characters.**")
  .replace("Style is 100% English, ≤700 chars", "Style is 100% English, target 760-900 chars, hard <=1000 chars");
