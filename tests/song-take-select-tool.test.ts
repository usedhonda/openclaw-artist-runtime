  ...
  it("requires exact adoption fields and records the durable adopted conversation", async () => {
    ...
    registerSongTools({
      registerTool(factory: (context: unknown) => typeof definition) {
        definition = factory({ workspaceDir: "/workspace", senderIsOwner: true, sessionKey: "session", messageChannel: "telegram", deliveryContext: { channel: "telegram", to: "1" } }) as typeof definition;
      }
    });
    ...
    expect(updateProductionConversation).toHaveBeenCalledWith("/workspace", expect.objectContaining({ senderIsOwner: true }), expect.objectContaining({ songId: "song-001", runId: "run-old", phase: "adopted", pendingDecision: "", acceptedTake: { runId: "run-old", takeId: "take-a" } }));
  });
