  ...
  const runAction = async (action: "pause" | "resume" | "run-cycle" | "ideate") => {
    ...
    try {
      if (action === "pause") {
        ...
        const result = await apiPost<RunCycleResult>("/run-cycle");
        const feedback = runCycleFeedback(result);
        showErrorToast("runtime", feedback.reason, feedback.message);
      }
      await refresh();
    } catch (caughtError) {
      const message = caughtError instanceof Error ? caughtError.message : String(caughtError);
